...
import { Check, Star, Calendar, CreditCard, Zap, Crown } from 'lucide-react'
...
export default function PricingSubscriptionBold({ content }: PricingSubscriptionProps) {
  const { data } = content

  if (!data) {
    return <div>No data provided for Pricing Subscription Bold.</div>
  }

  return (
    <section className="relative py-20 md:py-28 bg-gradient-to-br from-green-900 via-blue-900 to-purple-900 text-white overflow-hidden">
      {/* Background Elements */}
          ...
          <div className="bg-white/10 backdrop-blur-sm rounded-xl p-1 flex border border-white/20">
            {data.billingOptions.map((option, index) => (
              <div
                key={index}
                className={cn(
                  "px-6 py-3 rounded-lg text-sm font-bold flex items-center space-x-2 transition-all duration-300",
                  option.active
